...
TEMP="${PWD}"
echo "[+] Seding build version"
sed -i".bk" "s/#BUILD/$BUILD_VERSION/g" "$TEMP/build-templates/stag/index.ejs"
echo "[+] Copy build template for env-$ENV"
cp -fr "$TEMP/build-templates/stag/index.ejs"  "$TEMP/build-templates"
